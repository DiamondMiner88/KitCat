    ...
    if (subcommand === 'help') {
      ...
      if (subcommand2 === 'help') {
        var embed = new Discord.MessageEmbed()
          ...
          .addField(
            'add',
            `Add an image to the blacklist. **Requires an attachment thats an image**\n\`${pfx}blacklist image add {optional: reason}\``
          )
          .addField('list', `List all the blacklisted images\n\`${pfx}blacklist image list\``)
          .addField(
            'remove',
            `Remove a image from the blacklist. **Requires an attachment thats an image**\n\`${pfx}blacklist image remove\``
          );
        message.channel.send(embed);
        ...
        imageHash(url, 16, true, (hashErr, hash) => {
          ...
          db.run(
            'INSERT OR IGNORE INTO image_blacklist(hash, guild, user, reason, url) VALUES(?, ?, ?, ?, ?)',
            [hash, message.guild.id, message.author.id, reason, url],
            (err) => {
        ...
        var embed = new Discord.MessageEmbed().setColor(0x0099ff).setTitle('Blacklisted images:');

        db.all(
          'SELECT hash, user, reason, url FROM image_blacklist WHERE guild=?',
          [message.guild.id],
          (err, results) => {
            if (err) {
        ...
        if (message.attachments.size === 0) {
          message.author.send(
            "You didn't provide an image attachment to remove from the blacklist."
          );
          message.delete();
          return;
        }
      ...
      message.channel.send(
        `You didnt provide a valid subcommand! Do \`${pfx}blacklist help\` for subcommands`
      );
  }
};
