...
export default class Snipe extends Command {
  ...
  async invoke(message: Message): Promise<any> {
    ...
    message.reply(new MessageEmbed().setAuthor(msg.tag, msg.avatarURL).setDescription(msg.content).setTimestamp(msg.createdAt));
...
export function addMessageToSnipeCache(message: Message | PartialMessage) {
  ...
  snipeCache.set(message.channel.id, new StoredMessage(message), 21600 /* 6 hours */);
}
