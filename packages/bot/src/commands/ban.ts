...
export default class extends Command {
  ...
  async invoke(message: Discord.Message, args: string[]): Promise<any> {
    if (!hasPermission(message.member!, Permissions.FLAGS.BAN_MEMBERS))
    ...
    if (!t.bannable)
      return message.channel.send(
        `I don't have the permission to ban that user! Check my permissions, or my role in the hierarchy.`
      );
