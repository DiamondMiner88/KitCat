...
export default class extends Module {
  ...
  async invoke(interaction: CommandInteraction, options: Collection<string, any>) {
    const rawAmount: number = options.get('amount');
    const amount = clamp(rawAmount, 1, 100);
    (interaction.channel as TextChannel).bulkDelete(amount, true).then(
      messages => {
        // TODO: log to logChannel
        interaction.client.setTimeout(() => interaction.reply(`Deleted ${messages.size} messages.`, { ephemeral: true }), 2000);
      },
      error => interaction.client.setTimeout(() => interaction.reply(`Failed to purge because of ${error.message}`, { ephemeral: true }), 2000),
    );
  }
}
