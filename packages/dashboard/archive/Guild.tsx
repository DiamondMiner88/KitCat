...
import { FormControl, Snackbar, Tooltip, Button, FormLabel, FormHelperText, TextField } from '@material-ui/core';
...
export default function Guild(props: any) {
    ...
    const addError = (error: string) => setErrors((errors) => errors.concat([error]));
    const [saveStatus, setSaveStatus] = React.useState<'saved_no_popup' | 'saved_open_popup' | 'unsaved' | 'no_popup'>(
        'saved_no_popup'
    );
    ...
    useEffect(() => {
        ...
        if (cookies.get('access-token')) {
            ...
            fetchData();
        } else window.history.pushState('', '', process.env.PUBLIC_URL + '/login');
    }, [guildID, settings]);
