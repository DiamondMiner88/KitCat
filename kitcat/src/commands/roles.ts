...
export class Roles extends Command {
  ...
  run(message: Message, args: string[], settings: IGuildSettings) {
    ...
    let roles: Record<string, Snowflake> = {};
    ...
    function update() {
      const e = new MessageEmbed()
        ...
        .addField(
          '2) Done',
          'Reacts to your message with the added emojis and listens for future reacts'
        )
      ...
      if (message.author.id === m.author.id) {
        if (adding_role == 1) {
          ...
          if (m.mentions.roles.size !== 1)
            return message.channel.send('You need to mention 1 role!');
    ...
    setTimeout(() => {
      if (active) {
        message.channel.send(
          `${message.author}, it has been 10 minutes so I have automatically canceled.`
        );
        collector.stop();
      }
    }, 10 * 60 * 1000);
...
export async function onMessageReactionAdd(reaction: MessageReaction, user: PartialUser | User) {
  try {
    ...
    console.log('reaction partial error');
    console.log(error);
    return;
  }
  ...
  if (reaction.emoji instanceof ReactionEmoji && msgSelfRoles[reaction.emoji.name])
    reaction.message.member.roles.add(msgSelfRoles[reaction.emoji.name]).catch(r => {
      reaction.message.author.send(r).catch(() => {});
    });
