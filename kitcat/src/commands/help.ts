...
export default class Help extends Command {
    ...
    invoke(message: Discord.Message, args: string[]): any {
        ...
        if (args.length === 0) {
            ...
            const target: typeof CategoriesData[0] | Command | undefined = CategoriesData.find(c => c.name === args[0]) || commands.find(c => c.trigger === args[0]);
            ...
            if (target instanceof Command) {
                ...
                commands.filter(c => CategoriesData[c.category].name === target.name && !c.unlisted).forEach(c => embed.addField(c.name, c.description + '\n' + c.formattedUsage));
            }
        }
