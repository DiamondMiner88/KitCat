...
export default class NameMC extends Command {
    ...
    description = `See players's skin, username history, and a link to thier NameMC.`;
    usage = '{username}';
    guildOnly = false;
    unlisted = false;
    nsfw = false;
                ...
                fetch(`https://api.mojang.com/user/profiles/${user.id}/names`)
                    ...
                    .then(names => {
                        ...
                        names.forEach((name: any) => {
                            ...
                            const date = 'temp';
                            embed.addField(Discord.Util.escapeMarkdown(name.name), Discord.Util.escapeMarkdown(name.changedToAt ? `${date} UTC` : 'Initial username'));
                        });
