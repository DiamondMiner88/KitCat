...
api.post('/guilds/:guild/save', async (request, response) => {
  ...
  try {
    ...
    LOGGER.error(
      'Error sending request to discord.com to verify access token. api-1.0.0 /guild/:guild/save'
    );
    LOGGER.error(error.message);
    return response.json({
      status: 5,
...
api.post('/guilds', async (request, response) => {
  ...
  try {
    ...
    LOGGER.error(
      'Error sending request to discord.com to verify access token/get guilds. api-1.0.0 /guilds'
    );
    LOGGER.error(error.message);
    return response.json({
      status: 5,
