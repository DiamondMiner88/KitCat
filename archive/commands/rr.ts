...
export async function getMessageRRSet(message: Message): Promise<RRSet | undefined> {
  ...
  let fetched = await prisma.rRSet.findFirst({ where: { message: message.id } });
...
export default class extends Command {
  ...
  name = 'Reaction Roles';
  description = `Create a message that when reacted to, assigns you a role.`;
  usage = '';
  guildOnly = true;
  unlisted = false;
  nsfw = false;
  ...
  invoke(message: Message): any {
    ...
    const newTimeout = () => {
      ...
      last_timeout_id = message.client.setTimeout(() => {
              ...
              return m.channel
                ...
                .then(() => {
                  ...
                  m.channel
                    ...
                    .then(m => message.client.setTimeout(() => m.delete().catch(NOOP), 5000));
                  clearInterval(last_timeout_id);
                })
